...
public final class UpdateNoticeCheck {
	...
	private static void report(MinecraftClient client, UpdateNotice notice) {
		...
		MutableText msg = Text.literal("[ShopLogger] ").formatted(ChatFormat.PREFIX)
				.append(Text.literal("Update available: ").formatted(ChatFormat.SUCCESS))
				.append(Text.literal(notice.message).formatted(ChatFormat.RESULT));
