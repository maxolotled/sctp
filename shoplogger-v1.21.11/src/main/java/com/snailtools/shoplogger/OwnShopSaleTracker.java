...
public final class OwnShopSaleTracker {
	...
	public static void check(MinecraftClient client, ShopSign sign, BlockPos containerPos, ScreenHandler handler) {
	...
	private static boolean containerHasItem(ScreenHandler handler, String baseItemId) {
		if (!(handler instanceof GenericContainerScreenHandler containerHandler)) return false;
		int invSize = containerHandler.getInventory().size();
		for (int i = 0; i < invSize && i < handler.slots.size(); i++) {
			ItemStack stack = handler.getSlot(i).getStack();
			if (stack == null || stack.isEmpty()) continue;
			if (baseItemId.equalsIgnoreCase(Registries.ITEM.getId(stack.getItem()).toString())) return true;
		}
		return false;
	}
}
